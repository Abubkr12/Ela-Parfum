...
export default function PesananKustomDetailPage() {
  ...
  useEffect(() => {
    if (request && (request.status === 'pending' || request.status === 'quoted')) {
      ...
      const pctBibit = rasio === '100/0' ? 1.0 : rasio === '70/30' ? 0.7 : rasio === '50/50' ? 0.5 : 0.3;
  ...
  const fetchDetail = async () => {
    try {
      ...
      const parsedRatio = ['100/0', '70/30', '50/50', '30/70'].includes(dbRatio) ? dbRatio : '50/50';
      ...
      if (reqData.status === 'pending' && !reqData.price_perfume) {
        ...
        const pctBibit = parsedRatio === '100/0' ? 1.0 : parsedRatio === '70/30' ? 0.7 : parsedRatio === '50/50' ? 0.5 : 0.3;
        const mlBibit = volume * pctBibit;
        const mlPelarut = volume * (1 - pctBibit);
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
        <Link href="/admin/pesanan" className="btn btn-outline" style={{ padding: 8, borderRadius: "var(--r-full)" }}>
          <ArrowLeft size={18} />
        </Link>
        <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 350px", gap: 24 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
          ...
          <div className="card mobile-pad-reduce" style={{ padding: "32px", border: "1px solid var(--c-border)", boxShadow: "0 10px 40px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.05)", background: "var(--c-surface-2)", overflow: "hidden", position: "relative" }}>
          ...
          <div className="card mobile-pad-reduce" style={{ padding: "32px", border: "1px solid var(--c-border)", boxShadow: "0 10px 40px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.05)", background: "var(--c-surface-2)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
          ...
          <div className="card mobile-pad-reduce" style={{ padding: "32px", background: "var(--glass-bg)", backdropFilter: "blur(20px)", border: "1px solid var(--c-border)", boxShadow: "var(--shadow-float)" }}>
